...
Cleaned text:"""
    ...
    def extract_chapters_from_pdf(self, pdf_path: str, progress_callback: Optional[Callable] = None) -> List[Dict]:
        """
        Extract chapters from PDF with improved text extraction using PDFMiner
        """
        ...
        # Try extraction methods in order of quality (PDFMiner first)
        extraction_methods = [
            ("PDFMiner with layout analysis", self._extract_with_pdfminer_advanced),
            ("PDFMiner simple extraction", self._extract_with_pdfminer_simple),
            ("PyMuPDF with batching", self._extract_with_pymupdf_batched),
            ...
            try:
                ...
                if full_text and len(full_text.strip()) > 500:
                    ...
                    if chapters:
                        ...
                        logger.warning(f"No chapters found using {method_name}")
                else:
                    logger.warning(f"Insufficient text extracted using {method_name}")
    ...
    def _extract_with_pdfminer_advanced(self, pdf_path: str, progress_callback: Optional[Callable] = None) -> Tuple[str, int]:
        """
        Advanced PDFMiner extraction with layout analysis - best for complex PDFs
        """
        full_text = ""
        pages_processed = 0
        ...
        try:
            ...
            laparams = LAParams(
                line_margin=0.5,        # Margin for grouping lines
                word_margin=0.1,        # Margin for grouping words  
                char_margin=2.0,        # Margin for grouping characters
                box_flow=0.5,           # Flow threshold for text boxes
                detect_vertical=False,   # Don't detect vertical text (for now)
            )
            ...
            # Find story start page
            start_page = self._find_story_start_page_pdfminer(pdf_path)
            logger.info(f"Processing {total_pages} pages starting from page {start_page}")
            
            # Extract text page by page with layout analysis
            with open(pdf_path, 'rb') as file:
                ...
                for page_num, page in enumerate(pages[start_page:], start_page):
                    try:
            ...
            logger.error(f"Error in PDFMiner advanced extraction: {e}")
            raise
        
        logger.info(f"PDFMiner advanced extraction completed: {pages_processed} pages processed")
        return full_text, pages_processed
    
    def _extract_with_pdfminer_simple(self, pdf_path: str, progress_callback: Optional[Callable] = None) -> Tuple[str, int]:
        """
        Simple PDFMiner extraction - fast and reliable for most PDFs
        """
        try:
            logger.info("Using PDFMiner simple extraction")
            
            # Extract all text at once (faster for simple cases)
            ...
            with open(pdf_path, 'rb') as file:
                pages_processed = len(list(PDFPage.get_pages(file)))
            
            # Find and remove content before story starts
            cleaned_text = self._clean_extracted_text_pdfminer(full_text)
            
            logger.info(f"PDFMiner simple extraction completed: {pages_processed} pages")
            return cleaned_text, pages_processed
            
        except Exception as e:
            logger.error(f"Error in PDFMiner simple extraction: {e}")
            raise
    ...
    def _clean_extracted_text_pdfminer(self, text: str) -> str:
        ...
        # Remove content before story likely starts
        lines = text.split('\n')
        ...
        for i, line in enumerate(lines):
            for pattern in story_start_patterns:
                if re.search(pattern, line):
                    story_start_line = max(0, i - 2)  # Include a bit before
                    logger.info(f"Found story start at line {i}: {line[:50]}...")
                    break
            if story_start_line > 0:
                break
        
        # If no clear start found, remove first portion that's likely front matter
        if story_start_line == 0:
            # Remove first 10% of lines or look for substantial content
            for i, line in enumerate(lines[:len(lines)//5]):  # Check first 20%
            ...
            if story_start_line == 0:
                story_start_line = len(lines) // 10  # Remove first 10%
        
        # Take text from story start
